...
let
  ...
  assertListOfStrings =
    label: val:
    if lib.isList val && lib.all lib.isString val then
      val
    else
      builtins.throw "${label} must be a list of strings";

  assertString =
    label: val: if lib.isString val then val else builtins.throw "${label} must be a string";
  ...
  validate =
    ...
    let
      ...
      compose =
        if svc ? compose && svc.compose != null then
          {
          ...
          null;
      backup =
        if svc ? backup && svc.backup != null then
          {
